...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `fenrir` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@fenrir/shared/git`, `@fenrir/shared/DrainableWorker`) — no barrel index.
