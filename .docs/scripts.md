...
- Dev commands default `FENRIR_STATE_DIR` to `~/.fenrir/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.fenrir-2`
...
- Desktop production windows load the bundled UI from `fenrir://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `fenrir` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `FENRIR_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `FENRIR_DEV_INSTANCE`)
- Example: `FENRIR_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `FENRIR_PORT_OFFSET` to a numeric offset.
