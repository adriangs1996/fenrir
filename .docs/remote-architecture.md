...
This document describes the target architecture for first-class remote environments in Fenrir.
...
- Keep the Fenrir server as the execution boundary.
...
Fenrir already has a clean runtime boundary: the client talks to a Fenrir server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Fenrir server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Fenrir server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Fenrir runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Fenrir server instance.
...
How does the client speak WebSocket to a Fenrir server?
...
- best fit for hosted or self-managed remote Fenrir deployments
...
For Fenrir, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
- reach a machine
- probe it
- launch or reuse a remote Fenrir server
...
How does a Fenrir server come to exist on the target machine?
...
The user or operator already runs Fenrir on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Fenrir:
...
The recommended Fenrir flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Fenrir availability.
3. Desktop launches or reuses a remote Fenrir server.
...
This is the inverse of remote launch: a local Fenrir server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Fenrir server might be reached through direct `wss`.
...
Fenrir already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Fenrir should not copy that part.

Fenrir already has the right runtime boundary:

- one Fenrir server per environment
- ordinary HTTP/WebSocket between client and environment

So Fenrir should borrow Zed's launch discipline, not its transport protocol.
