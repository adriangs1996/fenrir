...
Fenrir has one server-side observability model:
...
```bash
export FENRIR_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export FENRIR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export FENRIR_OTLP_SERVICE_NAME=fenrir-local
```

Optional:

```bash
export FENRIR_TRACE_MIN_LEVEL=Info
export FENRIR_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `FENRIR_OTLP_*`.

macOS app bundle example:

```bash
FENRIR_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
FENRIR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
FENRIR_OTLP_SERVICE_NAME=fenrir-desktop \
"/Applications/Fenrir.app/Contents/MacOS/Fenrir"
```
...
```bash
tail -f "$FENRIR_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$FENRIR_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$FENRIR_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `fenrir-local`, `fenrir-dev`, or `fenrir-desktop`
...
- `FENRIR_OTLP_TRACES_URL` was not set
...
- `FENRIR_TRACE_FILE`: override trace file path
- `FENRIR_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `FENRIR_TRACE_MAX_FILES`: rotated file count, default `10`
- `FENRIR_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `FENRIR_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `FENRIR_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `FENRIR_OTLP_TRACES_URL`: OTLP trace endpoint
- `FENRIR_OTLP_METRICS_URL`: OTLP metric endpoint
- `FENRIR_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `FENRIR_OTLP_SERVICE_NAME`: service name, default `fenrir-server`
