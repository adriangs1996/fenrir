...
export const DEFAULT_FENRIR_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".fenrir"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@fenrir/contracts",
    "--filter=@fenrir/web",
    "--filter=fenrir",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=fenrir"],
  "dev:web": ["run", "dev", "--filter=@fenrir/web"],
  "dev:desktop": ["run", "dev", "--filter=@fenrir/desktop", "--filter=@fenrir/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("FENRIR_PORT_OFFSET"),
  devInstance: optionalStringConfig("FENRIR_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid FENRIR_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `FENRIR_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric FENRIR_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed FENRIR_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_FENRIR_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly fenrirHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(fenrirHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      FENRIR_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.FENRIR_PORT = String(serverPort);
      ...
      delete output.FENRIR_MODE;
      delete output.FENRIR_NO_BROWSER;
      delete output.FENRIR_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.FENRIR_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.FENRIR_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.FENRIR_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.FENRIR_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.FENRIR_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.FENRIR_MODE = "web";
      delete output.FENRIR_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read FENRIR_PORT_OFFSET/FENRIR_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("FENRIR_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("FENRIR_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.FENRIR_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.FENRIR_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  fenrirHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Fenrir data (equivalent to FENRIR_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("FENRIR_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to FENRIR_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("FENRIR_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to FENRIR_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("FENRIR_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to FENRIR_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("FENRIR_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to FENRIR_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("FENRIR_PORT")),
  ),
