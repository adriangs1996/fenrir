...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit FENRIR_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "FENRIR_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid FENRIR_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults FENRIR_HOME to ~/.fenrir when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FENRIR_HOME, resolve(homedir(), ".fenrir"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FENRIR_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.FENRIR_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.FENRIR_NO_BROWSER, "1");
        assert.equal(env.FENRIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.FENRIR_LOG_WS_EVENTS, "1");
        assert.equal(env.FENRIR_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            FENRIR_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          fenrirHome: undefined,
        ...
        assert.equal(env.FENRIR_MODE, "web");
        assert.equal(env.FENRIR_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FENRIR_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom fenrirHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.FENRIR_HOME, resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FENRIR_MODE, undefined);
        assert.equal(env.FENRIR_NO_BROWSER, undefined);
        assert.equal(env.FENRIR_HOST, undefined);
