...
export type WorkflowReferenceSection =
  | "overview"
  | "ctx"
  | "examples"
  | "capabilities"
  ...
  [
    {
      name: "ctx.step",
      signature:
        "ctx.step(stepKey: string, fn: () => Promise<T> | T): Promise<T>",
      description:
      ...
      example:
        'const plan = await ctx.step("plan", () => planner.ask("Create a plan."));',
    },
    {
      name: "ctx.parallel",
      signature:
      ...
      description:
        "Runs bounded parallel workflow work without exposing raw worker or process APIs.",
    ...
    },
    {
      ...
      description:
        "Appends an auditable workflow log event visible in the run timeline.",
      capabilities: ["workflow.events"],
      events: ["workflow.notification.emitted"],
      errors: ["Fails when the event payload cannot be serialized."],
      example:
        'await ctx.log({ phase: "research", filesConsidered: files.length });',
    },
    {
      name: "ctx.notify",
      signature:
        "ctx.notify(input: { level: string; title: string; body?: string }): Promise<void>",
      description:
        "Emits a user-visible workflow notification and timeline event.",
      capabilities: ["workflow.notifications"],
      events: ["workflow.notification.emitted"],
      errors: ["Fails when title is empty."],
      example:
        'await ctx.notify({ level: "info", title: "Workflow completed" });',
    },
    {
      name: "ctx.team.agent",
      signature:
      ...
      errors: [
        "Fails when the agent name or role is empty, or when the provider cannot start.",
      ],
      example:
    ...
    },
    {
      name: "agent.ask",
      signature:
        "agent.ask(promptOrInput: string | { prompt: string }): Promise<{ text: string }>",
      description:
      ...
      errors: [
        "Fails when the provider turn fails, is cancelled, or times out.",
      ],
      example:
    ...
    },
    {
      ...
      description:
        "Lists active or historical workflow memory items available to context planning.",
      capabilities: ["workflow.memory"],
      events: [],
      errors: ["Fails when filter values are invalid."],
      example:
        'const hints = await ctx.memory.list({ kind: "prompt_hint", status: "active" });',
    },
    {
    ...
    },
    {
      ...
      description:
        "Searches the current project workspace through Fenrir-controlled APIs.",
      capabilities: ["workspace.search"],
      events: ["workflow.capability.called"],
      errors: [
        "Fails when query is empty or requested globs are outside the project.",
      ],
      example:
        'const hits = await ctx.workspace.search({ query: "WorkflowService", limit: 20 });',
    },
    {
      name: "ctx.workspace.readFile",
      signature: "ctx.workspace.readFile(path: string): Promise<string>",
      description:
        "Reads a project workspace file through capability checks and audit logging.",
      capabilities: ["workspace.read"],
      events: ["workflow.capability.called"],
      errors: [
        "Fails when the file is outside the project or exceeds size limits.",
      ],
      example:
    ...
    },
    {
      ...
      signature:
        "ctx.fs.writeFile(path: string, contents: string): Promise<void>",
      description:
        "Writes an explicitly permitted file path. This is never granted by default and must be declared.",
      capabilities: ["fs.write"],
      events: ["workflow.capability.called"],
      errors: [
        "Fails when write capability is missing or the path is outside the grant.",
      ],
    ...
    },
    {
      ...
      example:
        'await ctx.state.update("attempts", (current) => Number(current ?? 0) + 1);',
    },
    {
      name: "ctx.notes.add",
      signature:
      ...
      example:
        'await ctx.notes.add({ title: "Risk", body: "Migration touches public contracts." });',
    },
    {
      name: "ctx.tasks.propose",
      signature:
        'ctx.tasks.propose(input: { title: string; reason?: string; kind?: "research" | "analysis" | "review" | "implementation" | "other"; assignee?: string; prompt: string }): Promise<WorkflowTask>',
      description:
        "Creates a workflow task proposal for policy-driven acceptance or rejection.",
      capabilities: ["workflow.tasks"],
      events: ["workflow.task.proposed"],
      errors: ["Fails when title or prompt is invalid."],
      example:
    ...
    },
    {
      ...
      example: "await ctx.tasks.accept(taskId);",
    },
    {
      name: "ctx.tasks.reject",
      signature:
        "ctx.tasks.reject(taskId: string, reason?: string): Promise<void>",
      ...
      example: 'await ctx.tasks.reject(taskId, "Out of scope for this run.");',
    },
    {
      name: "ctx.tasks.run",
      signature: "ctx.tasks.run(taskId: string): Promise<unknown>",
      description:
        "Runs an accepted task through the assignee agent or workflow policy.",
      ...
      errors: [
        "Fails when the task is not accepted or the assignee turn fails.",
      ],
...
export function buildWorkflowReferenceMarkdown(
  section?: WorkflowReferenceSection,
): string {
  const parts: string[] = [
  ...
  if (sectionAllows(section, "ctx")) {
    parts.push(
      "## Context API",
      "",
      ...WORKFLOW_RUNTIME_API_REGISTRY.map(markdownForApiEntry),
      "",
    );
  }
...
export function buildWorkflowReferenceJson(
  section?: WorkflowReferenceSection,
): unknown {
  return {
    ...
    ...(sectionAllows(section, "overview")
      ...
      : {}),
    ...(sectionAllows(section, "ctx")
      ? { ctx: WORKFLOW_RUNTIME_API_REGISTRY }
      : {}),
    ...
    ...(sectionAllows(section, "errors")
      ...
      : {}),
    ...(sectionAllows(section, "examples")
      ? { examples: WORKFLOW_REFERENCE_EXAMPLES }
      : {}),
  };
}
