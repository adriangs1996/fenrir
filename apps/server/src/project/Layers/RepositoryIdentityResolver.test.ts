...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-test-",
      });
      ...
      expect(identity?.canonicalKey).toBe("github.com/fenrir/fenrir");
      expect(identity?.displayName).toBe("fenrir/fenrir");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("fenrir");
      expect(identity?.name).toBe("fenrir");
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/fenrir.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:Fenrir/fenrir.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-nested-group-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:Fenrir/platform/fenrir.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/fenrir/platform/fenrir");
      expect(identity?.displayName).toBe("fenrir/platform/fenrir");
      expect(identity?.owner).toBe("fenrir");
      expect(identity?.name).toBe("fenrir");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "fenrir-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/fenrir/fenrir");
        expect(refreshedIdentity?.name).toBe("fenrir");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/fenrir/fenrir");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:Fenrir/fenrir-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/fenrir/fenrir");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/fenrir/fenrir-next");
      expect(refreshedIdentity?.displayName).toBe("fenrir/fenrir-next");
      expect(refreshedIdentity?.name).toBe("fenrir-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
