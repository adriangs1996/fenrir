...
describe("VcsProjectConfig", () => {
  ...
  it.layer(TestLayer)("discovers .fenrir/vcs.json from nested workspaces", (it) => {
    it.effect("returns the configured kind", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "fenrir-vcs-config-test-",
        });
        const configDir = path.join(root, ".fenrir");
  ...
  it.layer(TestLayer)("keeps legacy .t3code/vcs.json compatibility", (it) => {
    it.effect("returns the configured kind", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "fenrir-legacy-vcs-config-test-",
        });
