...
function assertDescendantPid(
  ...
  if (pid === process.pid) {
    return Effect.fail(toProcessDiagnosticsError("Refusing to signal the Fenrir server process."));
  }
      ...
      return descendant
        ...
        : Effect.fail(
            toProcessDiagnosticsError(
              `Process ${pid} is not a live descendant of the Fenrir server.`,
            ),
          );
    }),
  );
...
export const ProcessDiagnosticsLive = Layer.effect(ProcessDiagnostics, make());
