...
import type { ModelSelection, ProviderRuntimeEvent, ProviderSession } from "@fenrir/contracts";
import {
  ApprovalRequestId,
  CommandId,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "fenrir-reactor-"));
    ...
    const layer = ProviderCommandReactorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(SourceControlWorkflows, {
          renameBranch,
        } as unknown as SourceControlWorkflowsShape),
      ),
      Layer.provideMerge(
        Layer.succeed(SourceControlStatus, {
  ...
  it("reacts to thread.context-compact-requested by calling provider compact", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
  ...
  it("rejects thread.context-compact-requested while the provider session is running", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-running-for-compact"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
