...
function canCompactContextForSession(session: OrchestrationSession): boolean {
...
const make = Effect.gen(function* () {
  ...
  const maybeGenerateAndRenameWorktreeBranchForFirstTurn = Effect.fn(
    ...
    yield* Effect.gen(function* () {
      ...
      const renamed = yield* sourceControlWorkflows.renameBranch({
        cwd,
        oldBranch,
      ...
      yield* sourceControlStatus.refreshStatus(cwd).pipe(Effect.ignoreCause({ log: true }));
    }).pipe(
  ...
  const processContextCompactRequested = Effect.fn("processContextCompactRequested")(function* (
    ...
    if (!canCompactContextForSession(thread.session)) {
      return yield* appendProviderFailureActivity({
        threadId: event.payload.threadId,
        kind: "provider.context.compact.failed",
        summary: "Context compaction failed",
        detail: `Provider session must be idle or ready before context compaction. Current status: ${thread.session.status}.`,
        turnId: null,
        createdAt: event.payload.createdAt,
      });
    }
