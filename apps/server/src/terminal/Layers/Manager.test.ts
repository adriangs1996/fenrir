...
it.layer(TestLayer, { excludeTestServices: true })("TerminalManager", (it) => {
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.FENRIR_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            FENRIR_PROJECT_ROOT: "/repo",
            FENRIR_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.FENRIR_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.FENRIR_WORKTREE_PATH, "/repo/worktree-a");
  ...
  it.effect("publishTmuxOutput pushes output events to subscribers", () =>
    Effect.gen(function* () {
      ...
      const events = yield* getEvents;
      const outputEvent = events.find((e) => e.type === "output" && e.threadId === "tmux:proj-1");
  ...
  it.effect("publishTmuxExit pushes exited events to subscribers", () =>
    Effect.gen(function* () {
      ...
      const events = yield* getEvents;
      const exitEvent = events.find((e) => e.type === "exited" && e.threadId === "tmux:proj-1");
