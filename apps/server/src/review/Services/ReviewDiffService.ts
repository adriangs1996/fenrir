...
import { Data, Duration, Effect, ServiceMap, Stream } from "effect";
...
export class ReviewDiffService extends ServiceMap.Service<
  ReviewDiffService,
  ReviewDiffServiceShape
...
function toGroupId(sessionId: ReviewSessionId, lane: ReviewRawLaneKind): ReviewGroupId {
  return ReviewGroupId.makeUnsafe(stableEntityId("review-group", [sessionId, lane]));
}
...
function toFileId(
  ...
  return ReviewFileId.makeUnsafe(stableEntityId("review-file", [sessionId, lane, normalizedPath]));
}
  ...
  const loadFilePatchArtifact: ReviewDiffServiceShape["loadFilePatchArtifact"] = (input) =>
    Effect.gen(function* () {
      ...
      const chunkArtifacts = splitPatchChunkArtifacts(patchResult.stdout).map((artifact) => {
        ...
        const chunkId = ReviewChunkId.makeUnsafe(
