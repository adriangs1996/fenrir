...
import type { GitStatusStreamEvent } from "../../../../../packages/contracts/src/git.ts";
import { GitCommandError } from "../../../../../packages/contracts/src/git.ts";
...
describe("ReviewDiffService", () => {
  ...
  it("debounces snapshot recomputation over status bursts", async () => {
    ...
    const sessionId = ReviewSessionId.makeUnsafe("review-session-2");
  ...
  it("surfaces review-ignored files in the ignored lane with their source provenance", async () => {
    ...
    const sessionId = ReviewSessionId.makeUnsafe("review-session-3");
