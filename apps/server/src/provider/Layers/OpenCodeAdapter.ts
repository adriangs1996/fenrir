...
export function makeOpenCodeAdapter(options?: OpenCodeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const resolveSettingsForInstance = (providerInstanceId: ProviderInstanceId) =>
      serverSettings.getSettings.pipe(
        Effect.mapError(
          (cause) =>
            new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "resolveOpenCodeInstanceSettings",
              issue:
                cause instanceof Error
                  ? cause.message
                  : `Failed to load OpenCode settings for '${providerInstanceId}'.`,
              cause,
            }),
        ),
        Effect.flatMap((settings) => resolveOpenCodeInstanceSettings(settings, providerInstanceId)),
      );
      ...
      function* (input) {
        ...
        if (!openCodeSettings.enabled) {
          return yield* new ProviderAdapterValidationError({
            ...
            issue: `OpenCode instance '${providerInstanceId}' is disabled in Fenrir settings.`,
          });
        }
...
export const OpenCodeAdapterLive = Layer.effect(OpenCodeAdapter, makeOpenCodeAdapter()).pipe(
  Layer.provideMerge(OpenCodeRuntimeLive),
);

export function makeOpenCodeAdapterLive(options?: OpenCodeAdapterLiveOptions) {
  return Layer.effect(OpenCodeAdapter, makeOpenCodeAdapter(options)).pipe(
    Layer.provideMerge(OpenCodeRuntimeLive),
  );
}
