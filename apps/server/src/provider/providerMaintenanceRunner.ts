...
function commandOutput(result: ProviderMaintenanceCommandResult): string | null {
  ...
  return output ? truncateText(output, UPDATE_OUTPUT_MAX_BYTES) : null;
}
            ...
            return yield* finish(
              makeUpdateState({
                ...
                message: couldNotVerify
                  ? "Update command completed, but Fenrir could not verify the provider version."
                  : stillOutdated
                    ? "Update command completed, but Fenrir still detects an outdated provider version."
                    : "Provider updated.",
                output: commandOutput(result),
              }),
            );
...
export const ProviderMaintenanceRunnerLive = Layer.effect(ProviderMaintenanceRunner, make());
