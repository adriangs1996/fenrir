...
} from "@fenrir/shared/git";

import { GitManagerError } from "@fenrir/contracts";
import {
  GitManager,
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `fenrir/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `fenrir-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
