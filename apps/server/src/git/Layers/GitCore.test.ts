...
function okGitProcessResult(stdout = "") {
      ...
      Effect.gen(function* () {
        ...
        const core = yield* makeIsolatedGitCore((input) => {
          ...
          if (input.operation === "GitCore.statusDetails.defaultRef") {
            return okGitProcessResult("refs/remotes/origin/main\n");
          }
          return Effect.fail(
            new GitCommandError({
              operation: input.operation,
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fenrir/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fenrir/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "fenrir/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "fenrir/tmp-working",
          newBranch: "fenrir/feat/session",
        });

        expect(renamed.branch).toBe("fenrir/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "fenrir/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "fenrir/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("fenrir/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fenrir/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fenrir/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "fenrir/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "fenrir/tmp-working" });
        ...
        expect(renamed.branch).toBe("fenrir/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:fenrir-org/fenrir.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:fenrir-org/fenrir.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "fenrir/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
