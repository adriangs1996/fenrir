  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "fenrir-web",
          attributes: {
            "service.runtime": "fenrir-web",
    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(PlanRunnerService)({
          ...
          resume: () => Effect.die(new Error("not available in test")),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "fenrir-router-project-favicon-fallback-",
      });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "fenrir-router-project-favicon-query-token-",
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "fenrir-web");
  ...
  it.effect("routes websocket rpc subscribeServerConfig streams snapshot then update", () =>
    Effect.gen(function* () {
      ...
      if (first?.type === "snapshot") {
        ...
        assert.equal(first.config.observability.diagnosticsDirectoryPath?.endsWith("/logs"), true);
  ...
  it.effect("enriches replayed project events with repository identity metadata", () =>
    Effect.gen(function* () {
      const repositoryIdentity = {
        canonicalKey: "github.com/fenrir/fenrir",
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: "git@github.com:Fenrir/fenrir.git",
        },
        displayName: "Fenrir/fenrir",
        provider: "github",
        owner: "Fenrir",
        name: "fenrir",
      };
    ...
    () =>
      Effect.gen(function* () {
        ...
        const createWorktree = vi.fn((_: Parameters<GitCoreShape["createWorktree"]>[0]) =>
          Effect.succeed({
            worktree: {
              branch: "fenrir/bootstrap-branch",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          branch: "main",
          newBranch: "fenrir/bootstrap-branch",
          path: null,
        });
