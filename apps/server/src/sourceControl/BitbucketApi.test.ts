...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitCoreShape["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:pingdotgg/t3code.git"),
    ),
    statusDetails: vi.fn<GitCoreShape["statusDetails"]>(() =>
      Effect.succeed({
        isRepo: true,
    ...
    ensureRemote: vi.fn<GitCoreShape["ensureRemote"]>(() => Effect.succeed("octocat")),
    fetchRemoteBranch: vi.fn<GitCoreShape["fetchRemoteBranch"]>(() => Effect.void),
    setBranchUpstream: vi.fn<GitCoreShape["setBranchUpstream"]>(() => Effect.void),
    checkoutBranch: vi.fn<GitCoreShape["checkoutBranch"]>((request) =>
      Effect.succeed({ branch: request.branch }),
    ),
    listLocalBranchNames: vi.fn<GitCoreShape["listLocalBranchNames"]>(() => Effect.succeed([])),
  };
  const git = {
    ...gitMock,
    ...input.git,
  ...
  const layer = BitbucketApi.layer.pipe(
    ...
    Layer.provide(
      Layer.mock(GitVcsDriver.GitVcsDriver)({
        ...
        detectRepository: () => Effect.die("detectRepository should not be called in this test"),
        isInsideWorkTree: () => Effect.die("isInsideWorkTree should not be called in this test"),
        listWorkspaceFiles: () =>
          Effect.die("listWorkspaceFiles should not be called in this test"),
        listRemotes: driver.listRemotes,
        filterIgnoredPaths: () =>
          Effect.die("filterIgnoredPaths should not be called in this test"),
        initRepository: () => Effect.die("initRepository should not be called in this test"),
      }),
    ),
    Layer.provide(Layer.mock(GitCore)(git)),
    Layer.provide(
...
it.effect("checks out same-repository pull requests with the existing Bitbucket remote", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.checkoutBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "feature/source-control",
    });
  }).pipe(Effect.provide(layer));
