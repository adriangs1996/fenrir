...
import { sanitizeBranchFragment } from "@fenrir/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@fenrir/shared/sourceControl";
...
export const make = Effect.fn("makeBitbucketApi")(function* () {
  ...
  const resolvePrimaryRemoteName = Effect.fn("BitbucketApi.resolvePrimaryRemoteName")(function* (
    cwd: string,
  ) {
    ...
    if (details?.branch) {
      const branchRemote = yield* readConfigValueNullable(cwd, `branch.${details.branch}.remote`);
      if (branchRemote) return branchRemote;
    }

    const originRemoteUrl = yield* readConfigValueNullable(cwd, "remote.origin.url");
    return originRemoteUrl ? "origin" : null;
  });
  ...
  return BitbucketApi.of({
    ...
    checkoutPullRequest: (input) =>
      Effect.gen(function* () {
        ...
        const localBranchNames = yield* gitCore.listLocalBranchNames(input.cwd);
        ...
        if (input.force === true || !localBranchExists) {
          ...
          yield* git.execute({
            operation: "BitbucketApi.checkoutPullRequest.fetchRemoteTrackingBranch",
            cwd: input.cwd,
            args: ["fetch", remoteName, remoteBranch],
          });
        }
        ...
        yield* gitCore.checkoutBranch({ cwd: input.cwd, branch: localBranch });
      }).pipe(
