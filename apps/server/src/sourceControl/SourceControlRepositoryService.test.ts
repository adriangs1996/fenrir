...
function processOutput(): VcsProcessOutput {
  return {
    exitCode: Number(ChildProcessSpawner.ExitCode(0)),
    stdout: "",
    stderr: "",
...
function makeLayer(input: {
  ...
  readonly git?: Partial<VcsDriverShape>;
  readonly gitCore?: {
  ...
  const gitDriver: VcsDriverShape = {
    capabilities: {
      kind: "git",
    ...
    listWorkspaceFiles: () => Effect.die("listWorkspaceFiles should not be called in this test"),
    listRemotes: () => Effect.die("listRemotes should not be called in this test"),
    filterIgnoredPaths: () => Effect.die("filterIgnoredPaths should not be called in this test"),
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(Layer.mock(GitVcsDriver.GitVcsDriver)(gitDriver)),
    Layer.provide(
      Layer.mock(GitCore)({
    ...
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), { prefix: "fenrir-source-control-repos-" }),
    ),
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "fenrir-source-control-clone-parent-",
    });
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(pushCalls, [{ cwd: "/workspace", remoteName: null }]);
  }).pipe(
    Effect.provide(
      makeLayer({
        provider,
        gitCore: {
          ...
          pushCurrentBranch: (cwd) =>
            Effect.sync(() => {
              pushCalls.push({ cwd, remoteName: null });
              return {
                status: "pushed" as const,
                branch: "feature/remote-v1",
                upstreamBranch: "origin/feature/remote-v1",
          ...
          pushCurrentBranch: (cwd) =>
            Effect.sync(() => {
              ...
              return {
                status: "pushed" as const,
                branch: "feature/remote-v1",
                upstreamBranch: "origin-1/feature/remote-v1",
                setUpstream: true,
              };
            }),
        },
