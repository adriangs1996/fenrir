...
export class ReviewSessionService extends Context.Service<
...
function normalizeBaseBranchOverride(value: string | null | undefined): string | null {
  ...
  return trimmed === null ? null : TrimmedNonEmptyString.make(trimmed);
}
  ...
  (input) =>
    Effect.gen(function* () {
      ...
      const nextSession: ReviewSessionRecord = reusableSession
        ...
        : {
            sessionId: ReviewSessionId.make(`review-session-${randomUUID()}`),
