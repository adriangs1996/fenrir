...
import { hashReviewAnchor } from "@fenrir/shared/sourceControlReview";
...
export class ReviewWriteService extends Context.Service<
  ReviewWriteService,
...
function toDraftSnapshot(input: {
  ...
  return {
    id: GitHubReviewDraftId.make("github-review-local-pending"),
    state: "pending",
    pullRequestNumber: input.pullRequestNumber,
    decision: reviewDraftDecision(input.drafts),
    ...(summaryDraft ? { body: summaryDraft.body } : {}),
    threads: inlineDrafts.map((draft) => {
      const threadId = GitHubReviewThreadId.make(`github-pending-thread-${draft.id}`);
      ...
      return {
        ...
        comments: [
          {
            id: GitHubReviewCommentId.make(`github-pending-comment-${draft.id}`),
            threadId,
            path: anchor.normalizedPath,
            body: draft.body,
            anchor,
    ...
    threads:
      input.remote.status === "available"
        ? input.remote.snapshot.reviewThreads.map((thread) => ({
            id: GitHubReviewThreadId.make(thread.id),
            path: thread.path,
            anchor: thread.anchor,
            isResolved: thread.isResolved,
            isOutdated: thread.isOutdated,
            comments: thread.comments.map((comment) => ({
              id: GitHubReviewCommentId.make(comment.id),
              threadId: GitHubReviewThreadId.make(thread.id),
              path: comment.path,
              body: comment.body,
  ...
  const upsertGitHubDraft: ReviewWriteServiceShape["upsertGitHubDraft"] = (input) =>
    Effect.gen(function* () {
      ...
      const nextDraft: ReviewGitHubPendingDraftType = {
        id:
          existingDraft?.id ??
          GitHubReviewDraftId.make(`github-review-draft-${dependencies.makeId()}`),
        sessionId: session.sessionId,
        authSessionId: input.authSessionId,
