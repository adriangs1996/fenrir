...
export function initVpnManager(stateDir: string): void {
  pidFilePath = Path.join(stateDir, "vpn.pid");
  cleanupOrphanedVpn();
}
...
export async function connectVpn(profile: VpnProfile): Promise<VpnConnectionState> {
  ...
  return new Promise<VpnConnectionState>((resolve) => {
    ...
    child.stdout?.on("data", (data: Buffer) => {
      ...
      // Extract assigned IP from ifconfig line
      const ipMatch = text.match(/ifconfig\s+\S+\s+(\d+\.\d+\.\d+\.\d+)/);
      if (ipMatch?.[1]) {
...
function spawnPrivileged(configPath: string): ChildProcess.ChildProcess | null {
  try {
    if (process.platform === "darwin") {
      ...
      // We wrap openvpn in a shell so stdout/stderr are captured
      return ChildProcess.spawn(
        "osascript",
        [
          "-e",
          `do shell script "openvpn --config '${configPath.replace(/'/g, "'\\''")}'  --verb 3" with administrator privileges`,
        ],
