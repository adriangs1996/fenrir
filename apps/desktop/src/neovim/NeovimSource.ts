...
import { createEmbeddedThemeRuntimeCommand } from "./themeRuntime";
...
function nvimStartupArgs(colorscheme?: string | undefined): string[] {
  const args = ["--embed"];
  try {
    args.push("--cmd", createEmbeddedThemeRuntimeCommand());
  } catch (error) {
    console.warn("[neovimSource] embedded theme runtime setup failed:", error);
  }
...
export class NeovimSource implements SceneSource {
  ...
  private themeSelection: NeovimThemeSelection | null = null;
  ...
  async setTheme(selection: NeovimThemeSelection): Promise<void> {
    ...
    if (!this.client || !this.started) return;
    try {
      await this.applyThemeSelection(selection);
      this.requestFullRepaint();
    } catch (error) {
      console.warn("[neovimSource] theme apply failed:", error);
    }
  }
  ...
  private async ensureStarted(): Promise<void> {
    ...
    try {
      ...
      try {
        await this.applyThemeSelection();
      } catch (e) {
        console.warn("[neovimSource] theme lua failed:", e);
      }
