...
function clampDrawerHeight(height: number): number {
  const safeHeight = Number.isFinite(height) ? height : getDefaultThreadTerminalHeight();
...
export function TerminalViewport({
  ...
  useEffect(() => {
    ...
    const eventThreadRef =
      mode === "tmux" && projectId
        ? { environmentId, threadId: `tmux:${projectId}` as ThreadId }
        : threadRef;
    const eventTerminalId = mode === "tmux" && projectId ? "tmux" : terminalId;
    ...
    const sendTerminalInput = async (data: string, fallbackError: string) => {
      ...
      try {
        if (mode === "tmux" && projectId) {
          await api.terminal.writeTmux({ projectId, data });
        } else {
    ...
    // Match terminal Device Attributes responses generated by xterm.js
    // (Primary DA: ESC[?...c, Secondary DA: ESC[>...c).
    // In tmux mode these arrive too late and appear as shell garbage.
    // oxlint-disable-next-line no-control-regex -- intentionally matching ESC control character in terminal data
    const DA_RESPONSE_RE = /^\u001b\[[?>]\d[\d;]*c$/;

    const inputDisposable = terminal.onData((data) => {
      if (mode === "tmux" && DA_RESPONSE_RE.test(data)) return;
      const writePromise =
        mode === "tmux" && projectId
          ? api.terminal.writeTmux({ projectId, data })
    ...
    const fitTimer = window.setTimeout(() => {
      ...
      void resizePromise.catch(() => undefined);
    }, 30);
  ...
  }, [drawerHeight, environmentId, mode, projectId, resizeEpoch, terminalId, threadId]);
  return (
    <div
      ref={containerRef}
...
export default function ThreadTerminalDrawer({
  ...
  const handleTerminalMount = useCallback((terminalId: string, terminal: Terminal | null) => {
    if (terminal) {
