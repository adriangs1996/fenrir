...
export const PROVIDER_UPDATE_DISMISSALS_STORAGE_KEY = "fenrir:provider-update-dismissals:v1";
...
function writeProviderUpdateDismissals(document: ProviderUpdateDismissals): void {
  try {
    ...
    // Dismissal state is best-effort UI state.
  }
}
...
export function isProviderUpdateNotificationDismissed(
  ...
  return dismissalKey ? readProviderUpdateDismissals().keys.includes(dismissalKey) : false;
}
...
export function useDismissedProviderUpdateNotificationKeys() {
  ...
  const dismissNotificationKey = useCallback(
    (key: string) => {
      ...
      if (!trimmedKey || dismissedKeySet.has(trimmedKey)) {
        return;
      }
