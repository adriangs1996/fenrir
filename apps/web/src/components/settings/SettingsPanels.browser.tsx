...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.fenrir-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [],
    availableEditors: ["cursor"],
    observability: {
      logsDirectoryPath: "/repo/project/.fenrir/logs",
...
const createDesktopBridgeStub = (overrides?: {
  ...
  return {
    ...
    trafficLensCreateTabInProfile: vi.fn().mockResolvedValue({
      tabId: "test-tab",
    ...
    trafficLensSetTabViewMode: vi.fn().mockResolvedValue({
      tabId: "test-tab",
      profileId: null,
      url: "about:blank",
      title: "",
    ...
    trafficLensSetTabMobilePreset: vi.fn().mockResolvedValue({
      tabId: "test-tab",
      profileId: null,
      url: "about:blank",
      title: "",
    ...
    trafficLensCreateRule: vi.fn().mockRejectedValue(new Error("not implemented")),
    trafficLensUpdateRule: vi.fn().mockRejectedValue(new Error("not implemented")),
    ...
    trafficLensCreateProfile: vi.fn().mockRejectedValue(new Error("not implemented")),
    trafficLensUpdateProfile: vi.fn().mockRejectedValue(new Error("not implemented")),
    ...
    trafficLensCreateOverride: vi.fn().mockRejectedValue(new Error("not implemented")),
    trafficLensUpdateOverride: vi.fn().mockRejectedValue(new Error("not implemented")),
    ...
    neovimSetTheme: vi.fn().mockResolvedValue(undefined),
    onNeovimRedraw: () => () => {},
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows diagnostics inside About with a single logs-folder action", async () => {
    ...
    await expect
      .element(page.getByText("/repo/project/.fenrir/logs", { exact: true }))
      .toBeInTheDocument();
    await expect
      .element(
        page.getByText(
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(page.getByText("Fenrir will restart to expose this environment over the network."))
      .toBeInTheDocument();
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.fenrir/logs", "cursor");
  });
});
