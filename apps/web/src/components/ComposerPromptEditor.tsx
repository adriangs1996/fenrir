...
function renderMentionChipDom(container: HTMLElement, pathValue: string): void {
  ...
  icon.src = getVscodeIconUrlForEntry(
    pathValue,
    inferEntryKindFromPath(pathValue),
    theme,
  );
...
function getAbsoluteOffsetForPoint(
  ...
  if (node instanceof ComposerTerminalContextNode || node instanceof ComposerMentionNode) {
...
function ComposerInlineTokenBackspacePlugin() {
  const [editor] = useLexicalComposerContext();
  const { onRemoveTerminalContext } = useContext(
...
>(function ComposerPromptEditor(
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "fenrir-composer-editor",
      editable: true,
