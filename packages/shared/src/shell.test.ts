...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__FENRIR_ENV_PATH_START__\n/a:/b\n__FENRIR_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__FENRIR_ENV_PATH_START__");
    expect(args?.[1]).toContain("__FENRIR_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__FENRIR_ENV_PATH_END__",
        "__FENRIR_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__FENRIR_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__FENRIR_ENV_CUSTOM_VAR_START__", "  padded value  ", "__FENRIR_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
