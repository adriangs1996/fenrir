/**
 * ANSI escape sequence sanitizer for terminal history persistence.
 *
 * Pure functions — no dependencies, no side effects. Strips device-query
 * sequences that cause terminal re-interrogation on replay while preserving
 * visual formatting (colors, cursor movement, etc.).
 *
...
/**
 * Incrementally sanitize a chunk of terminal output for history persistence.
 *
 * Strips device-query CSI/OSC sequences that would cause terminal
 * re-interrogation on history replay, while preserving visual formatting.
 *
 * Handles partial escape sequences at chunk boundaries via
 * `pendingControlSequence` carry.
 */
...
/**
 * Cap terminal history to a maximum number of lines, keeping the most recent.
 */
